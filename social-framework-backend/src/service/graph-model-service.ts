...
export interface IGraphModelService extends IModelService {
  ...
  findByRelations<T extends IModel>(modelName:string, relations:SearchRelation[]): Promise<T[]>;
...
export class GraphModelService extends ModelService<IGraphDBClient> implements IGraphModelService {
  ...
  public findByRelations<T extends IModel>(modelName:string, relations:SearchRelation[]): Promise<T[]> {
    return new Promise<T[]>((resolve, reject) => {
      this.dbClient.findByRelations<T>(modelName, relations, (error, results:T[]) => {
        if(error) {
          reject(error);
        }
        return resolve(results);
