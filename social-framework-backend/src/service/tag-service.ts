...
export interface ITagService extends IGraphModelService {
  ...
  removeTag(name: string, taggedModel: IModel): Promise<any>;
...
export class TagService extends GraphModelService implements ITagService {
  ...
  public removeTag(name: string, taggedModel: IModel): Promise<any> {
    return new Promise<any>((resolve, reject) => {
      this.getTagByName(name).then((existingTag) => {
        if(existingTag) {
          this.unrelate(taggedModel, 'HAS_TAG', existingTag).then(() => {
            console.log('Done relating tag to model');
            return resolve();
          }).catch((err) => {
            console.log('Error relating tag: ' + err);
        ...
        console.log(`Error retrieving tag ${name}: ${err}`);
        return reject(err);
      });
    });
  }
